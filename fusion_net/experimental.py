...
class MobileNetV3Block(nn.Module):
    def __init__(self, in_channels, 
        ...
        self.depthwise_conv = nn.Conv2d(in_channels * expand_ratio, in_channels * expand_ratio, kernel_size=kernel_size, stride=self.stride, padding=kernel_size//2, groups=in_channels * expand_ratio, bias=False)
        self.bn2 = nn.BatchNorm2d(in_channels * expand_ratio)

        # 3. Squeeze and Excitation 
        if self.use_se:
            self.se = SEBlock(in_channels * expand_ratio)
...
if __name__ == "__main__":
    # x = torch.rand(1, 2, 640, 640)
    input=torch.randn(1, 2, 480, 640)
    mbconv=MBConvBlock(ksize=3,
                       input_filters=2,
                       output_filters=16,
                       image_size=480)
    out=mbconv(input)
    print(out.shape)
    # model = MobileNetV3Block(in_channels=2, out_channels=16, kernel_size=3)
    # model = DepthwiseSeparableConv2d(in_channels=2, out_channels=16, kernel_size=3, padding=1)
    # model = SELayer(channel=2, reduction=1)
    # model = ChannelAttention(in_planes=2, ratio=1)
    # out = model(x)
    print(out.shape)